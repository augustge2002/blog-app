    ...
    <el-container class="me-ct-container">
      <el-main>
        <div class="me-ct-title me-area">
          ...
          <span class="me-ct-meta">{{ct.articles}} Posts</span>
        </div>
    ...
    computed: {
      title() {
        if(this.$route.params.type === 'tag'){
          return `${this.ct.tagName} - Tag`
        }
        return `${this.ct.categoryName} - Category`
      }
    },
    methods: {
      ...
      getCategoryDetail(id) {
        ...
        getCategoryDetail(id).then(data => {
          ...
          if (error !== 'error') {
            that.$message({type: 'error', message: 'Failed to Load Category', showClose: true})
          }
        })
      },
      getTagDetail(id) {
        ...
        getTagDetail(id).then(data => {
          ...
          if (error !== 'error') {
            that.$message({type: 'error', message: 'Failed to Load Tag', showClose: true})
          }
        })
      },
