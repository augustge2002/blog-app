    ...
    <el-container>
      <base-header :simple=true>
        <el-col :span="4" :offset="2">
          <div class="me-write-info">Make a post</div>
        </el-col>
      ...
      <el-dialog title="Summary Category Tag"
        ...
        <el-form :model="articleForm" ref="articleForm" :rules="rules">
          ...
          <el-form-item label="Categories" prop="category">
            <el-select v-model="articleForm.category" value-key="id" placeholder="Choose categories">
      ...
      return {
        ...
        rules: {
          summary: [
            ...
            {max: 80, message: 'No more than 80 characters', trigger: 'blur'}
          ],
          category: [
            {required: true, message: 'Please choose categories', trigger: 'change'}
          ],
          tags: [
            {type: 'array', required: true, message: 'Choose categories', trigger: 'change'}
          ]
        }
      }
    },
    ...
    methods: {
      getArticleById(id) {
        ...
        getArticleById(id).then(data => {
          ...
          if (error !== 'error') {
            that.$message({type: 'error', message: 'Failed to load post', showClose: true})
          }
        })
      },
      publishShow() {
        ...
        if (this.articleForm.title.length > 30) {
          this.$message({message: 'Title should be no longer than 30 characters', type: 'warning', showClose: true})
          return
        }

        if (!this.articleForm.editor.ref.d_render) {
          this.$message({message: 'Content should be non-empty', type: 'warning', showClose: true})
          return
        }
        ...
        this.$refs[articleForm].validate((valid) => {
          if (valid) {
            ...
            publishArticle(article,this.$store.state.token).then((data) => {
              if(data.success){
                loading.close();
                that.$message({message: 'Post success', type: 'success', showClose: true})
                that.$router.push({path: `/view/${data.data.id}`})
              }else{
                that.$message({message: error, type: 'Post failed:'+data.msg, showClose: true});
              }
      ...
      cancel() {
        this.$confirm('Post will not be saved, continue?', 'Prompt', {
        ...
        getAllCategorys().then(data => {
          if(data.success){
            that.categorys = data.data
             ...
             that.$message({type: 'error', message: 'Failed to load categories', showClose: true})
          }
        ...
        getAllTags().then(data => {
          if(data.success){
            that.tags = data.data
             ...
             that.$message({type: 'error', message: 'Failed to load tags', showClose: true})
          }
        }).catch(error => {
          if (error !== 'error') {
            that.$message({type: 'error', message: 'Failed to load tags', showClose: true})
